...
colnames(weekdate) <- c("date","年週")

##所有資料、全國每週、各縣市每週、全國累計
mask_count_view_re_sort_week <- left_join(mask_count_view_re_sort, weekdate, by="date")
mask_count_taiwan<-mask_count_view_re_sort_week %>% group_by(年週) %>% summarise(count = sum(count))%>% as.data.frame()
mask_count_single<-mask_count_view_re_sort_week %>% group_by(年週, counties) %>% summarise(count = sum(count)) %>% as.data.frame()
